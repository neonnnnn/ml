...
class FactorizationMachine(object):
    def __init__(self, k, reg=[1e-5, 1e-5, 1e-5], task='r', optimizer='sgd', iter=10000, lr=0.0001, sigma=0.01, rng=np.random.RandomState(1)):
        self.k = k
        self.reg = reg
        self.iter = iter
        self.lr = lr
    ...
    def decision_function(self, x):
        return self.bias + np.dot(x, self.w) + 0.5 * (np.sum(np.atleast_2d(np.dot(x, self.V)**2), axis=1) -
                                                      np.sum(np.atleast_2d(np.dot(x**2, self.V**2)), axis=1))

    @staticmethod
    def _sigmoid_crossentropy_loss(output, y):
        return - (y * np.log(1. / (1 + np.exp(-output))) + (1 - y) * np.log(1. - 1 / (1 + np.exp(-output))))
    ...
    def _grad_V_output(self, xi):
        return np.dot(np.atleast_2d(xi).T, np.atleast_2d(np.dot(xi, self.V))) - self.V * np.atleast_2d(xi ** 2).T
    ...
    def _sgd(self, x, y):
        for xi, yi in zip(x, y):
            ...
            self.V -= self.lr * (grad_loss_output * self._grad_V_output(xi) + self.reg[2] * self.V)
    ...
    def _als(self, x, y):
        ...
        for i in xrange(n):
            new_wi = (self.w[i] * x_squared_sum_axis0[i] + np.dot(x[:, i], e)) / (x_squared_sum_axis0[i] + self.reg[1])
        ...
        for i in xrange(self.k):
            ...
            for j in xrange(n):
                ...
                new_v = (self.V[j, i] * np.sum(grad_vji ** 2) + np.dot(grad_vji, e)) / (np.sum(grad_vji ** 2) + self.reg[2])
                q += (new_v - self.V[j, i]) * x[:, j]
                e += (new_v - self.V[j, i]) * grad_vji
                self.V[j, i] = new_v

    def fit(self, x, y):
        self.V = self.rng.normal(0, scale=self.sigma, size=(x.shape[1], self.k))
