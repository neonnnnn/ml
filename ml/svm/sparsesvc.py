...
class SparseSVC(SVC):
    def __init__(self,  C, kernel_name='linear', params=None, eps=1e-3,
                 tau=1e-12, max_iter=10000, wss='WSS1'):
        self.C = C
    ...
    def _working_set_selection1(self, grad_f_a, up_idx, low_idx, x, y):
        ...
        if not (idx1 in self.cache):
            self.cache[idx1] = self.K.calc(x, x[idx1])

        if not (idx2 in self.cache):
            self.cache[idx2] = self.K.calc(x, x[idx2])
