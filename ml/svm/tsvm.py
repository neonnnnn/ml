...
class TSVM(object):
    def __init__(self, C_unlabel, s=0., max_iter_tsvm=100, C=1.0, 
                 kernel_name='linear', params=None, eps=1e-4, tau=1e-12, 
                 max_iter=10000, wss='WSS3', sparse=False, iprint=True):
        ...
        self.flag = False
        self.sparse = sparse
        self.clf = svc.SVC(C, kernel_name, params, eps, tau, max_iter, wss, 
                           sparse, iprint=False)
        self.L = None
        self.U = None
    ...
    def decision_function(self, x):
        if self.support_vector.shape[0] == self.alpha.shape[0]:
            K = (self.K.calc(self.support_vector, x))
        else:
            K = self.K.calc(self.support_vector, x)
    ...
    def wss(self, x, y, grad, up_idx, low_idx):
    ...
    def fit(self, x_labeled, y, x_unlabeled):
        ...
        for j in range(self.max_iter2):
            ...
            for i in xrange(self.max_iter):
                idx1, idx2, d = self.wss(x, y_all, grad, up_idx.nonzero()[0],
                                         low_idx.nonzero()[0])
                y1 = y_all[idx1]
                y2 = y_all[idx2]
