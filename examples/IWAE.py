...
class IWAE(VAE):
    ...
    def forward(self, x, train):
        if train:
            ...
            log_likelihood_q = self.encoder.log_likelihood(z, q_mean, q_logvar)
            if isinstance(decoder, (tuple, list)):
                log_likelihood_p = self.decoder.log_likelihood(rep_x, *decoder)
            else:
                log_likelihood_p = self.decoder.log_likelihood(rep_x, decoder)
            log_likelihood_p += self.encoder.log_likelihood(z, T.zeros(z.shape), T.ones(z.shape))
