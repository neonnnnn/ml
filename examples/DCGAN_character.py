...
def train_dcgan_character():
    ...
    concat_g.compile(batch_size=batch_size, nb_epoch=1, loss=CrossEntropy(),
                     opt=Adam(lr=0.0002, beta1=0.5))
    ...
    for i in xrange(n_epoch):
        ...
        for j in xrange(data_size/batch_size):
            ...
            for l in xrange(batch_size):
                img = loadimg(train_data[idx[l]])
                X_train[l] = 2.*img/255. - 1.
            # train discriminator
        ...
        # evaluation
        z = samplez(valid_size, 100)
        fake_valid = g.predict(z)
        acc_real = d.accuracy(X_valid, np.ones(valid_size).astype(np.int8))
        acc_fake = d.accuracy(fake_valid, np.zeros(valid_size).astype(np.int8))
        ...
        if (i+1) % 1 == 0:
            ...
            utils.color_saveimg(generation, (8, 8), filename)
            del generation


if __name__ == '__main__':
    train_dcgan_character()
