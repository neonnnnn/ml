...
if __name__ == '__main__':
    ...
    for l in xrange(5000):
        img = np.asarray(Image.open(StringIO(train_data[l])).convert('RGB')).astype(np.float32).transpose(2, 0, 1)
    ...
    z_plot = np.random.uniform(low=-1, high=1, size=400 * 100).reshape(400, 100).astype(np.float32)
    # train
    for i in xrange(n_epoch):
        ...
        for j in xrange(data_size/batch_size):
            ...
            for l in xrange(batch_size):
                img = np.asarray(Image.open(StringIO(train_data[idx[l]])).convert('RGB')).astype(np.float32).transpose(2, 0, 1)
        ...
        # validation
        z = np.random.uniform(low=-1, high=1, size=5000*100).reshape(5000, 100).astype(np.float32)
        imitation_valid = generator.predict(z)
        sys.stdout.write(' Real ACC:%.3f' % discriminator.accuracy(X_valid, np.ones(5000).astype(np.int8)))
        sys.stdout.write(' Gene ACC:%.3f' % discriminator.accuracy(imitation_valid, np.zeros(5000).astype(np.int8)))
        ...
        if (i+1) % 1 == 0:
            ...
            utils.color_saveimg(generation, (20, 20), "imgs/DCGAN/DCGAN_album_epoch" + str(i+1) + ".png")
        # generate Analogy
        if (i+1) % 50 == 0:
            ...
            utils.color_saveimg(generation, (10, 10), "imgs/DCGAN/DCGAN_album_Analogy_epoch" + str(i + 1) + ".png")
